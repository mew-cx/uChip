...
uint32_t USBHost::InTransfer(EpInfo *pep, uint32_t nak_limit, uint8_t *nbytesptr, uint8_t* data) {
	...
	*nbytesptr = 0;
	...
	//Serial.print("The buffer address is: 0x ");
	...
	while (1) {
		...
		//Serial.print("The ADDR in register is: 0x ");
		//Serial.println(usb_pipe_table[pep->epAddr].HostDescBank[0].ADDR.reg, HEX);
		rcode = dispatchPkt(tokIN, pep->epAddr, nak_limit); //IN packet to EP-'endpoint'. Function takes care of NAKS.
		//Serial.print("dispatchPkt tokIN error = 0x");
	...
	uhd_freeze_pipe(pep->epAddr);
	//Serial.print("PSTATUS = 0x");
	//Serial.println(USB->HOST.HostPipe[pep->epAddr].PSTATUS.reg, HEX);
	return ( rcode);
}
...
uint32_t USBHost::OutTransfer(EpInfo *pep, uint32_t nak_limit, uint32_t nbytes, uint8_t *data) {
	...
	while(bytes_left) {
		...
		//Serial.print("dispatchPkt tokOUT error = 0x");
		//Serial.println(rcode,HEX);
		if (rcode)
		{
			switch(rcode) {
...
uint32_t USBHost::dispatchPkt(uint32_t token, uint32_t epAddr, uint32_t nak_limit) {
	...
	{
		//Serial.print("PSTATUS = 0x");
		//Serial.println(USB->HOST.HostPipe[epAddr].PSTATUS.reg,HEX);
